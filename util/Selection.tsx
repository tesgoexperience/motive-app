...
class Selection extends Component<PropType, StateType> {
    ...
    renderUserList() {
        ...
        return <ScrollView showsVerticalScrollIndicator={false} style={{ width: '100%' }}>{usersListArray.map((k) => {
            let text = <Text style={{ fontWeight: '500', marginLeft: 20, fontSize: 20 }}>{k}</Text>
            let button;
    ...
    public render() {
        return < View style={{ marginTop: 40, paddingBottom: 30 }}>
            <Text style={{ fontWeight: 'bold', fontSize: 20, textAlign: 'left', marginBottom: 20 }}>{this.props.title}</Text>
